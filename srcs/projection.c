...
/*   Updated: 2019/10/14 15:51:36 by jcanteau         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (r < fdf->map.nbl)
	{
		...
		while (c < fdf->map.nbcol)
		{
			if(fdf->map.tab[r][c].z == 0)
				fdf->map.tab[r][c].z = 1;
			fdf->pro[r][c].px = fdf->cx + ((fdf->map.tab[r][c].x * fdf->fl) / (fdf->map.tab[r][c].z));
			fdf->pro[r][c].py = fdf->cy + ((fdf->map.tab[r][c].y * fdf->fl) / (fdf->map.tab[r][c].z));
